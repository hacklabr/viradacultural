...
class html {
     ...
     *
     * Para usar cache de parte, passe no array a chave '__CACHE' com um inteiro que representa o número de segundos em que o cache expira
     * @param string $slug
     * 
     ...
     * @example part('slug', array('__CACHE'=>3600, 'var1'=>'valor da var'); cacheia a parte por 1 hora
     */
    static function part($slug) {
        ...
        if($___use_cache){
            ...
            if(DCache::exists(__METHOD__, $___cache_id, $_part_vars['__CACHE'])){
                echo DCache::get(__METHOD__, $___cache_id);
                return;
            }
        }
        $slug = 'parts/' . $slug;
    ...
    static function getVideo($video_url, $width = '100%', $height = '100%') {
        ...
        if ($video_type == 'vimeo') {
            ...
            ob_start();
            ?>
            <iframe width="<?php echo $width ?>" height="<?php echo $height ?>" src="http://www.youtube.com/embed/<?php echo $video_id; ?>" frameborder="0" allowfullscreen></iframe>
            <?php
            return ob_get_clean();
        } else {
            return null;
